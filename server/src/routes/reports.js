...
router.get('/work-activity',asyncHandler(async(req,res)=>{
  ...
  const eventsRequest=queryAll(`SELECT a.*,u.name user_name,l.contact_name,l.company_name,l.phone FROM activity_events a JOIN users u ON u.id=a.user_id LEFT JOIN leads l ON l.id=a.lead_id WHERE ${eventWhere.join(' AND ')} ORDER BY datetime(a.created_at) DESC,a.id DESC`,eventParams);
  const docScope=userId?'p.created_by=?':'1=1',docParams=userId?[userId]:[];
  const ordersRequest=queryAll(`SELECT p.id,p.lead_id,p.po_number,p.total,p.advance_amount,p.pending_amount,p.created_at,p.created_by user_id,u.name user_name,l.contact_name,l.company_name FROM purchase_orders p JOIN users u ON u.id=p.created_by JOIN leads l ON l.id=p.lead_id WHERE ${docScope} AND p.manual_invoice=0 AND p.status<>'CANCELLED' AND datetime(p.created_at)>=datetime(?) AND datetime(p.created_at)<datetime(?)`,[...docParams,from,to]);
  const invoiceScope=userId?'i.created_by=?':'1=1',invoiceParams=userId?[userId]:[];
  const invoicesRequest=queryAll(`SELECT i.id,i.invoice_number,i.created_at,i.created_by user_id,u.name user_name,p.lead_id,p.total,l.contact_name,l.company_name FROM tax_invoices i JOIN users u ON u.id=i.created_by JOIN purchase_orders p ON p.id=i.purchase_order_id JOIN leads l ON l.id=p.lead_id WHERE ${invoiceScope} AND datetime(i.created_at)>=datetime(?) AND datetime(i.created_at)<datetime(?)`,[...invoiceParams,from,to]);
  const [events,orders,invoices]=await Promise.all([eventsRequest,ordersRequest,invoicesRequest]);
